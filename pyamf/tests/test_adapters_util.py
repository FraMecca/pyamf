...
class HelperTestCase(unittest.TestCase):
    ...
    def test_to_list(self):
        self.assertEqual(
            util.to_list(Iterable([1, 2, 3]), self.encoder),
            [1, 2, 3]
        )
    ...
    def test_to_set(self):
        self.assertEqual(
            util.to_set(Iterable([1, 2, 3]), self.encoder), set([1, 2, 3])
        )
        self.assertEqual(
            util.to_set(['a', 'b'], self.encoder), set(['a', 'b'])
        )
    ...
    def test_to_dict(self):
        self.assertEqual(
            util.to_dict(Iterable({'a': 'b'}), self.encoder), {'a': 'b'}
        )
    ...
    def test_to_tuple(self):
        self.assertEqual(
            util.to_tuple(Iterable((1, 2, 3)), self.encoder), (1, 2, 3)
        )
